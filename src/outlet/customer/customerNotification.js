...
const CustomerNotification = () => {
    ...
    const notificationByCustomer = useSelector(state => {
        console.log(state)
        return state.notification.notificationByCustomer;
    })
    useEffect(() => {
        dispatch(getCustomerByAccountLogin(account.id));
    },[])
    useEffect(() => {
        dispatch(getAllNotificationsByReceiverCustomer(customerLogin.id));
    },[customerLogin])
    //phan trang
    const [currentPage, setCurrentPage] = useState(1);
    const productsPerPage = 6;
    const totalProducts = notificationByCustomer.length;
    const totalPages = Math.ceil(totalProducts / productsPerPage);

    // Tạo danh sách sản phẩm cho trang hiện tại
    const startIndex = (currentPage - 1) * productsPerPage;
    const endIndex = startIndex + productsPerPage;
    const currentProducts = notificationByCustomer.slice(startIndex, endIndex);
                ...
                <div className="py-6 p-md-6 p-lg-10">
                    ...
                    <div className="table-responsive-xxl border-0">
                        ...
                        <table className="table mb-0 text-nowrap table-centered ">
                            ...
                            <tbody>
                            {currentProducts && currentProducts.map(notification => (
                                <tr>
